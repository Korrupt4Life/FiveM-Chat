...
window.APP = {
  ...
  watch: {
    messages() {
      ...
      //close chat header
      document.getElementById("butoaneSchimbareChat").style.display = "none";
	...
	messagesSTAFF() {
      ...
      const messagesObj = this.$refs.messagesSTAFF;
      this.$nextTick(() => {
        messagesObj.scrollTop = messagesObj.scrollHeight;
      });
    },
  ...
  methods: {
    ...
    ON_OPEN() {
      document.getElementById("butoaneSchimbareChat").style.display = "block";
      this.showInput = true;
      this.showWindow = true;
      this.showTab = true;
    ...
    ON_MESSAGESTAFF({ message }) {
      this.messagesSTAFF.push(message);
    },
    ON_MESSAGEM({ message }) {
    ...
    send(e) {
      ...
      if(msg !== '') {
        if (channel == 'global') {
          ...
          $.post('http://chat/chatResultSTAFF', JSON.stringify({
            message: getEmojiChars(msg),
          }));
          this.oldMessages.unshift(msg);
          msg = '';
...
function openPage(pageName, elmnt, evt) {
  ...
  elmnt.style.backgroundColor = "rgb(53, 114, 255)";
